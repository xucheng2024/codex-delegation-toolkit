...
Use these labels exactly and in this order. Fill every field; use `None — <reason>` only when genuinely inapplicable.
...
Allow at most two focused delta rounds by default. Each delta must name the blocker, smallest missing fact, and expected effect. Do not resend unchanged fields. After two unresolved deltas, report the gap and let the parent rescope or explicitly expand the budget.
